...
你只能输出合法 JSON，不要输出 markdown，不要输出解释，不要输出代码块,不要输出任何代码块标记，严格按照以下固定格式输出。
...
- 指令包含拿取含义时，如果没有指定要拿给谁，则默认拿给当前用户，返回点为用户所在的位置，如果有指定要拿给谁，则返回点为人物所在位置，如果指定拿到哪里，则返回点为该地点。特别地，如果包含“放回去”、“归位”、“放回原处”等含义时，返回点为物体的默认存放地（即名称为物体的地点）,请不要返回start_position,如果命令为“拿回来”，则返回start_position。
...
- 没有明确指定对象时自行理解对象是谁。
- 默认当前用户所在的位置都为start_position。
- 当拿取指令中指定了物品的具体位置（如“点位1的苹果”）时，导航至该具体位置（如“点位1”）。
