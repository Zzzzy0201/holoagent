...
- id: 字符串，必须唯一
- robot_id: 整数,默认为机器人1
- skill: 只能是{self.skill_registry}中的技能, 请仔细阅读每个技能的“适用场景”和“使用规则”来判断当前任务需要调用哪个技能。
- target：只能从对应技能下方列出的`allowed_targets`中选择一个，禁止凭空添加和使用不在列表中的词。
- depends_on: 字符串数组，没有依赖时必须输出 []
- “同时”表示并行，不要互相依赖
- “然后”“之后”“完成后”“到达后”表示建立依赖
- 如果用户没有具体的动作要求且表示了自己的需求时，请根据用户的需求和当前场景分析并执行能满足用户需求的动作。如果用户有具体的动作要求，请不要凭空添加用户未提及的动作,也不要曲解成相似的动作。
