...
- 指令包含拿取含义时，如果没有指定要拿给谁，则默认拿给当前用户，返回点为用户所在的位置，如果有指定要拿给谁，则返回点为人物所在位置，如果指定拿到哪里，则返回点为该地点。特别地，如果包含“放回去”、“归位”、“放回原处”等含义时，返回点为物体的默认存放地。
...
- 没有明确指定对象时自行理解对象是谁。
- 默认当前机器人和用户所在的位置都为start_position。
